    ...
    for i in range(len(spans_html)):
        ...
        try:
            if (findnth(span, 'Components', i) != -1):
                spell["components"] = componentsProcess(span[findnth(span, 'Components ', i) + 11:
                                                             findnth(span, 'EffectRange', i)])
                # print("components:", span[findnth(span, 'Components ', i)+11:findnth(span, 'EffectRange', i)])
        except:
            spell["components"] = "NA"
        try:
            if (findnth(span, 'Components', i) != -1):
                spell["spell_resistance"] = True if ("Spell Resistance" in span[findnth(span, 'Effect', i)
                                                                                :findnth(span, 'Description',
                                                                                         i)]) else False
